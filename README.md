...
| `GET /v1/comparer?sirens=` | $0.12/co | Compare 2–5 French companies side by side (identity, financials, score) — with guards against false-but-plausible comparisons (different fiscal years, consolidated vs social accounts, holdings). Every ranking carries a status in `eligibilite_classements`: the evaluative risk ranking is served only when the batch is comparable and the ranked scores rest on the same axes; otherwise it is absent, with its reasons |
