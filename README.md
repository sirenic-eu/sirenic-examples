...
| `GET /v1/entreprise/{siren}/capital` | $0.35 | Ownership from public articles, AI-extracted: corporate holders named, natural-person shareholders counted and never named (since 2026-09-19); an extraction that counts natural persons is kept 90 days at most |
...
| `GET /v1/kyb/{siren}` | $0.15 | Full KYB file + sanctions screening + commercial-court decisions (Judilibre, since 2026-09-24) |
...
| `GET /v1/documents/{type}/{id}` | $0.10 | Download a filed document (PDF): balance sheets (`bilans`) over x402; articles and minutes (`actes`) with an API key only, since 2026-09-19 (401 `compte_requis` without one, no quote) |
